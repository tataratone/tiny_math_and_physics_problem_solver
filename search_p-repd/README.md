...
すべて非自明な $`p_1-{\rm repd}`$ かつ $`p_2-{\rm repd}`$ かつ $`...`$ かつ $`p_k-{\rm repd}`$ であるような $`N`$ と $`k`$ 個の素数組 $`p_1, p_2, ... , p_k`$ を探したい．
...
| キー | 型 | 説明 |
|------|-----|------|
| `primes` | tuple[int, ...] | $`k`$ 個の素数の組 $`(p_1, p_2, ..., p_k)`$ |
| `digits` | tuple[int, ...] | 各素数 $`p_i`$ の桁数 $`d_i`$ |
| `a` | tuple[int, ...] | 各素数 $`p_i`$ のゾロ目の桁値 $`a_i`$ |
| `N` | int | $N$ の $10$ 進数表記  |
...
一番上の `{'primes': (2, 5), 'digits': (5, 3), 'a': (1, 1), 'N': 31}` は，$`N=31`$ を $2$ 進数で表記すると $`11111_{(2)}`$（$`5`$ 桁，桁値 $`1`$）で，$`5`$ 進数で表記すると $`111_{(5)}`$（$`3`$ 桁，桁値 $`1`$）．実際にこれは正しい．
